...
const api = useApi()
const { data: recentData } = await useAsyncData('articles-recent', () =>
  api.getArticles({ published: 1, per_page: LIST_LIMIT })
)
const { data: allData } = await useAsyncData('articles-all', () =>
...
<template>
  ...
  <section class="toc-list">
    <article v-if="recentArticles[0]" class="toc-cover-story">
      ...
      <div class="toc-cover-story-body">
        ...
        <h2 class="display toc-cover-title">
          <NuxtLink :to="`/articles/${recentArticles[0].slug}`">{{ recentArticles[0].title }}</NuxtLink>
        </h2>
    ...
    <!-- All articles link -->
    <NuxtLink to="/articles/library" class="toc-more label">Browse all articles →</NuxtLink>
  </section>
  ...
  <ScrollReveal v-if="hasCategory('技术')">
    <section class="col-tech">
      ...
      <div v-if="articlesByCategory['技术'].length > CAT_LIMIT" class="col-more">
        <NuxtLink to="/articles/library?cat=%E6%8A%80%E6%9C%AF" class="col-more-link label">View all {{ articlesByCategory['技术'].length - CAT_LIMIT }} more →</NuxtLink>
      </div>
    </section>
  </ScrollReveal>
  ...
  <ScrollReveal v-if="hasCategory('阅读')" :delay="120">
    <section class="col-read">
      ...
      <div v-if="articlesByCategory['阅读'].length > CAT_LIMIT" class="col-more">
        <NuxtLink to="/articles/library?cat=%E9%98%85%E8%AF%BB" class="col-more-link label">View all {{ articlesByCategory['阅读'].length - CAT_LIMIT }} more →</NuxtLink>
      </div>
    </section>
  </ScrollReveal>
  ...
  <ScrollReveal v-if="hasCategory('故事')" :delay="240">
    <section class="col-story">
      ...
      <div v-if="articlesByCategory['故事'].length > CAT_LIMIT" class="col-more">
        <NuxtLink to="/articles/library?cat=%E6%95%85%E4%BA%8B" class="col-more-link label">View all {{ articlesByCategory['故事'].length - CAT_LIMIT }} more →</NuxtLink>
      </div>
    </section>
  </ScrollReveal>
  ...
  <ScrollReveal v-if="hasCategory('随笔')" :delay="360">
    <section class="col-essay">
      ...
      <div v-if="articlesByCategory['随笔'].length > CAT_LIMIT" class="col-more">
        <NuxtLink to="/articles/library?cat=%E9%9A%8F%E7%AC%94" class="col-more-link label">View all {{ articlesByCategory['随笔'].length - CAT_LIMIT }} more →</NuxtLink>
      </div>
    </section>
  </ScrollReveal>
</template>
