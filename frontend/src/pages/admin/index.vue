  ...
  <div class="dashboard">
    ...
    <div class="dash-cards">
      <div class="dash-card">
        ...
        <router-link to="/admin/articles/create" class="dash-card-action label">写新文章 →</router-link>
      </div>
      <div class="dash-card">
        <span class="dash-card-icon">✎</span>
        <span class="dash-card-num">{{ articles.length + projects.length }}</span>
        <span class="dash-card-label label">写作工作台</span>
        <router-link to="/admin/workbench" class="dash-card-action label">打开写作台 →</router-link>
      </div>
      <div class="dash-card">
        <span class="dash-card-icon">◧</span>
        <span class="dash-card-num">{{ projects.length }}</span>
        <span class="dash-card-label label">项目 ({{ publishedProjects }} 已发布)</span>
        <router-link to="/admin/projects/create" class="dash-card-action label">添加项目 →</router-link>
      </div>
    </div>
    ...
    <div class="dash-section">
      ...
      <div v-if="articles.length" class="dash-list">
        <div v-for="a in articles.slice(0, 5)" :key="a.id" class="dash-list-item">
          ...
          <router-link :to="`/admin/articles/${a.id}`" class="dash-list-title">{{ a.title }}</router-link>
